...
export async function listAllPlaybills() {
  const client = await getAuthClient();
  const files = await listPlaybillFiles(client);
  
  return Array.from(files.entries()).map(([slug, { name }]) => ({
    slug,
    name: unslugify(name),
  }));
}

export async function listAllPlaybillsWithConfig() {
  const client = await getAuthClient();
  const files = await listPlaybillFiles(client);
  
  const playbills = [];
  for (const [slug, { id, name }] of files.entries()) {
    try {
